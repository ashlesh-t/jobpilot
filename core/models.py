...
Design rules:
  * No secrets. API keys and tokens live in the OS keyring (or .env) via
    scripts/jp_secrets.py. The `settings` table only records *which* secrets exist.
...
class Profile(Base):
    """A resume-derived candidate profile. One row is active at a time (latest verified)."""
...
class Resume(Base):
    """An uploaded resume file. Exactly one row has is_active=True."""
    ...
    __table_args__ = (
        UniqueConstraint("folder", "filename", name="uq_resume_folder_filename"),
...
class Scan(Base):
    ...
    __table_args__ = (Index("ix_scans_started", "started_at"),)
...
# --------------------------------------------------------------------------- #
class Job(Base):
    """A discovered job. `job_id` is the Layer A SHA1 of company|role|location."""
    __tablename__ = "jobs"
...
class Application(Base):
    """The user's application to a job, with a dated status trail."""
...
class TailoredResume(Base):
    """A resume tailored to one job. Files live under resumes/tailored/<folder_name>/."""
...
class CostEntry(Base):
    ...
    __table_args__ = (Index("ix_cost_ts", "ts"), Index("ix_cost_run", "run_id"))


class UserFeedback(Base):
    """Ported from schema/init.sql — drives the learning loop."""
...
class ChatMessage(Base):
    ...
    __table_args__ = (Index("ix_chat_conv", "conversation_id", "created_at"),)
