...
def _payload() -> dict:
    return {
        "slots": schedule_repo.list_all(),
        "jobs": scheduler.jobs(),
        "upcoming": scheduler.next_runs(7),
        "running": scheduler.running,
        "catchup_grace_hours": settings_repo.get("catchup_grace_hours", CATCHUP_GRACE_HOURS),
        "service": {
...
@router.post("/slots")
async def create_slot(req: SlotCreate):
    try:
        schedule_repo.create(name=req.name, time=req.time, timezone=req.timezone,
...
async def update_slot(slot_id: int, req: SlotUpdate):
    try:
        updated = schedule_repo.update(slot_id, **req.model_dump(exclude_none=True))
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    if updated is None:
...
async def delete_slot(slot_id: int):
    if not schedule_repo.delete(slot_id):
        raise HTTPException(status_code=404, detail="no such schedule slot")
    scheduler.reconfigure()
    return _payload()
...
@router.put("/catchup")
async def set_catchup(req: GracePatch):
    ...
    settings_repo.set("catchup_grace_hours", req.hours, export=False)
    return _payload()
...
async def service_control(action: str):
    ...
    return {"ok": ok, "service": _payload()["service"]}
